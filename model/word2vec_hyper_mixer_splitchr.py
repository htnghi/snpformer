...
class Mixer_each_chr(nn.Module):
    def __init__(self, embed_dim, maxlen, num_patches, tuning_params):
        ...
        self.mixer_blocks = torch.nn.ModuleList([
            Mixer(num_patches, maxlen, embed_dim, tuning_params)
...
class MixerModel(nn.Module):
    def __init__(self, embed_dim, maxlen, tuning_params):
        ...
        self.mixer_chr1 = Mixer_each_chr(embed_dim, maxlen[0], num_patches[0], tuning_params)
        self.mixer_chr2 = Mixer_each_chr(embed_dim, maxlen[1], num_patches[1], tuning_params)
        self.mixer_chr3 = Mixer_each_chr(embed_dim, maxlen[2], num_patches[2], tuning_params)
        self.mixer_chr4 = Mixer_each_chr(embed_dim, maxlen[3], num_patches[3], tuning_params)
        self.mixer_chr5 = Mixer_each_chr(embed_dim, maxlen[4], num_patches[4], tuning_params)
...
def objective(trial, list_X_train, src_vocab_size, y, data_variants, training_params_dict, avg_stop_epochs, device):
    ...
    tuning_params_dict = {
        ...
        'num_mixer_layers': trial.suggest_int("num_mixer_layers", 2, 6, step=1),
        # 'embedding_dim': trial.suggest_int("embedding_dim", 40, 256, step=4),
        'hyper_mixing_mlp_factor': trial.suggest_float('hyper_mixing_mlp_factor', 0.1, 0.8, step=0.05),
        'feature_mixing_mlp_factor': trial.suggest_float('feature_mixing_mlp_factor', 5, 20, step=0.5),
    ...
    for fold, fold_indices in enumerate(zip(*all_folds), start=1):
        ...
        try:
            model = MixerModel(embed_dim, max_seq_lens, tuning_params=tuning_params_dict).to(device)
...
# ==============================================================
def tuning_splitchr_hypermixer_word2vec(datapath, list_X_train, y, data_variants, training_params_dict, device):
...
# ==============================================================
def evaluate_result_splitchr_hypermixer_word2vec(datapath, list_X_train, y_train, list_X_test, y_test, best_params, data_variants, device):
    ...
    # create model
    model = MixerModel(embed_dim, max_seq_lens, tuning_params=best_params).to(device)
