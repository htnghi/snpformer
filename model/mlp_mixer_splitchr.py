...
class Mixer_each_chr(nn.Module):
    def __init__(self, vocab_size, num_patches, embedding_dim, tuning_params):
        ...
        self.mixer_blocks = torch.nn.ModuleList([
            Mixer(num_patches, embedding_dim, tuning_params)
    ...
    def forward(self, x):
        ...
        x = self.embedding(x)
        # print('shape after embedding', x.shape) #torch.Size([32, 23, 64])
        x = x.permute(0, 2, 1) # Change shape to (batch_size, embedding_dim, maxlen) for Conv1D
        # x = self.conv1d(x) #torch.Size([32, 64, 4])
...
class MixerModel(nn.Module):
    def __init__(self, vocab_size, maxlen, tuning_params):
        ...
        self.mixer_chr1 = Mixer_each_chr(vocab_size, num_patches[0], embedding_dim, tuning_params)
        self.mixer_chr2 = Mixer_each_chr(vocab_size, num_patches[1], embedding_dim, tuning_params)
        self.mixer_chr3 = Mixer_each_chr(vocab_size, num_patches[2], embedding_dim, tuning_params)
        self.mixer_chr4 = Mixer_each_chr(vocab_size, num_patches[3], embedding_dim, tuning_params)
        self.mixer_chr5 = Mixer_each_chr(vocab_size, num_patches[4], embedding_dim, tuning_params)
    ...
    def forward(self, list_X):
        ...
        mixer_output1 = self.mixer_chr1(list_X[0])
        mixer_output2 = self.mixer_chr2(list_X[1])
        mixer_output3 = self.mixer_chr3(list_X[2])
        mixer_output4 = self.mixer_chr4(list_X[3])
        mixer_output5 = self.mixer_chr5(list_X[4])

        concatenated_output = self.concatenate_chr([mixer_output1, mixer_output2, mixer_output3, mixer_output4, mixer_output5])
        # print('shape after Normalization in MixerModel', concatenated_output.shape) #torch.Size([32, 4, 64])
...
def objective(trial, list_X_train, src_vocab_size, y, data_variants, training_params_dict, avg_stop_epochs, device):
    ...
    tuning_params_dict = {
        ...
        'token_mixing_mlp_factor': trial.suggest_float('token_mixing_mlp_factor', 0.1, 0.8, step=0.05),
...
# ==============================================================
def tuning_splitchr_mlpmixer(datapath, list_X_train, src_vocab_size, y, data_variants, training_params_dict, device):
...
# ==============================================================
def evaluate_result_splitchr_mlpmixer(datapath, list_X_train, src_vocab_size, y_train, list_X_test, y_test, best_params, data_variants, device):
