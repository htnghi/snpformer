...
class TransformerSNP_infinite(torch.nn.Module):
    def __init__(self, device, src_vocab_size, seq_len, tuning_params):
        super(TransformerSNP_infinite, self).__init__()
...
class EnsembledModel(torch.nn.Module):
    def __init__(self, device, src_vocab_size, max_seq_lens, tuning_params):
        super(EnsembledModel, self).__init__()
        self.src_vocab_size = src_vocab_size
        self.tuning_params  = tuning_params
        self.transformer_chr1 = TransformerSNP_infinite(device, src_vocab_size, max_seq_lens[0], tuning_params)
        self.transformer_chr2 = TransformerSNP_infinite(device, src_vocab_size, max_seq_lens[1], tuning_params)
        self.transformer_chr3 = TransformerSNP_infinite(device, src_vocab_size, max_seq_lens[2], tuning_params)
        self.transformer_chr4 = TransformerSNP_infinite(device, src_vocab_size, max_seq_lens[3], tuning_params)
        self.transformer_chr5 = TransformerSNP_infinite(device,src_vocab_size, max_seq_lens[4], tuning_params)
        # max_seq_len = np.sum(max_seq_lens)
...
def objective(trial, list_X_train, src_vocab_size, y, data_variants, training_params_dict, avg_stop_epochs, device):
    ...
    for fold, fold_indices in enumerate(zip(*all_folds), start=1):
        ...
        try:
            model = EnsembledModel(device, src_vocab_size, max_seq_lens, tuning_params=tuning_params_dict).to(device)
...
# ==============================================================
def tuning_splitchr_transformer_test2_infinite(datapath, list_X_train, src_vocab_size, y, data_variants, training_params_dict, device):
...
# ==============================================================
def evaluate_result_splitchr_transformer_test2_infinite(datapath, list_X_train, src_vocab_size, y_train, list_X_test, y_test, best_params, data_variants, device):
    ...
    # create model
    model = EnsembledModel(device, src_vocab_size, max_seq_lens, tuning_params=best_params).to(device)
