...
class TransformerSNP(torch.nn.Module):
    def __init__(self, device, src_vocab_size, seq_len, max_seq_len, tuning_params):
        ...
        self.positional_encoding = embed_layer.PositionalEncoding(embedding_dimension, seq_len, tuning_params['dropout_transformer'])
...
def evaluate_result_splitchr_transformer(datapath, list_X_train, src_vocab_size, y_train, list_X_test, y_test, best_params, data_variants, device):
    ...
    print('--------------------------------------------------------------')
    print('Test Transformer - Chromosome-based results: avg_loss={:.4f}, avg_expvar={:.4f}, avg_r2score={:.4f}, avg_mae={:.4f}'.format(test_mse, test_exp_variance, test_r2, test_mae))
    print('--------------------------------------------------------------')
