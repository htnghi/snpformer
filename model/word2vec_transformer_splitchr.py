...
class TransformerSNP(torch.nn.Module):
    def __init__(self, device, seq_len, max_seq_len, embed_dim, tuning_params):
        ...
        self.positional_encoding = embed_layer.PositionalEncoding(embed_dim, seq_len, tuning_params['dropout'])
        # self.positional_encoding = embed_layer.PositionalEmbedding(embedding_dimension, seq_len, tuning_params['dropout'])
        self.encoder_blocks = torch.nn.ModuleList([
            encoder.EncoderChunk(device, embed_dim, tuning_params['n_heads'], tuning_params['mlp_factor'], tuning_params['dropout'], use_rope=False)
...
class RegressionBlock(torch.nn.Module):
    def __init__(self, tuning_params, embed_dim):
        ...
        self.linear  = Linear(in_features=embed_dim, out_features=1)
...
class EnsembledModel(torch.nn.Module):
    def __init__(self, device, seq_lens, max_seq_len, embed_dim, tuning_params):
        super(EnsembledModel, self).__init__()
        self.tuning_params  = tuning_params
        self.max_seq_len = max_seq_len

        self.transformer_chr1 = TransformerSNP(device, seq_lens[0], max_seq_len, embed_dim, tuning_params)
        self.transformer_chr2 = TransformerSNP(device, seq_lens[1], max_seq_len, embed_dim, tuning_params)
        self.transformer_chr3 = TransformerSNP(device, seq_lens[2], max_seq_len, embed_dim, tuning_params)
        self.transformer_chr4 = TransformerSNP(device, seq_lens[3], max_seq_len, embed_dim, tuning_params)
        self.transformer_chr5 = TransformerSNP(device, seq_lens[4], max_seq_len, embed_dim, tuning_params)
        # seq_len = np.sum(seq_lens)
        # self.transformer_block = TransformerSNP(src_vocab_size, seq_len, tuning_params)
        self.regression_block = RegressionBlock(tuning_params, embed_dim)
...
# ==============================================================
def objective(trial, list_X_train, y, data_variants, training_params_dict, avg_stop_epochs, device):
    ...
    for fold, fold_indices in enumerate(zip(*all_folds), start=1):
        ...
        # Apply word2vec
        X_train_list_w2v, trained_model_list_w2v = word2vec_embed_list(X_train_list)
        X_val_list_w2v, _ = word2vec_embed_list(X_val_list, word2vec_model=trained_model_list_w2v)
        ...
        seq_lens = []
        for i in range(len(X_train_list_w2v)):
            seq_lens.append(X_train_list_w2v[i].shape[1])
        
        max_seq_len= int(512)
        embed_dim = X_train_list_w2v[0].shape[2]

        set_seeds(seed=int(42+fold))
        try:
            model = EnsembledModel(device, seq_lens, max_seq_len, embed_dim, tuning_params=tuning_params_dict).to(device)
        ...
        try:
            y_pred, stopping_point = train_val_loop(model, training_params_dict, tuning_params_dict,
                                     X_train_list_w2v, y_train_scale, X_val_list_w2v, y_val_scale, device)
...
# ==============================================================
def tuning_splitchr_transformer_word2vec(datapath, list_X_train, y, data_variants, training_params_dict, device):
    ...
    study.optimize(lambda trial: objective(trial, list_X_train, y, data_variants, training_params_dict, avg_stopping_epochs, device), n_trials=num_trials)
    set_seeds()
...
# ==============================================================
def evaluate_result_splitchr_transformer_word2vec(datapath, list_X_train, y_train, list_X_test, y_test, best_params, data_variants, device):
    ...
    # Apply word2vec
    X_train_list_w2v, trained_model_list_w2v = word2vec_embed_list(list_X_train)
    X_test_list_w2v, _ = word2vec_embed_list(list_X_test, word2vec_model=trained_model_list_w2v)
    ...
    # create model
    model = EnsembledModel(device, seq_lens, max_seq_len, embed_dim, tuning_params=best_params).to(device)

    # transform data to tensor format
    list_tensor_X_train = [torch.from_numpy(item).long() for item in X_train_list_w2v]
    list_tensor_X_test = [torch.from_numpy(item).long() for item in X_test_list_w2v]
