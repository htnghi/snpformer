...
class TransformerSNP_infinite(torch.nn.Module):
    def __init__(self, src_vocab_size, max_seq_lens, tuning_params, device):
...
class EnsembledModel(torch.nn.Module):
    def __init__(self, src_vocab_size, max_seq_len, device, tuning_params):
        ...
        self.transformer_block = TransformerSNP_infinite(src_vocab_size, max_seq_len, tuning_params, device)
...
def objective(trial, X, src_vocab_size, y, data_variants, training_params_dict, avg_stop_epochs, device):
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(X, y)):
        ...
        try:
            model = EnsembledModel(src_vocab_size, max_seq_len, device, tuning_params=tuning_params_dict).to(device)
...
# ==============================================================
def tuning_regular_transformer_test2_infinite(datapath, X_train, src_vocab_size, y, data_variants, training_params_dict, device):
...
# ==============================================================
def evaluate_result_regular_transformer_test2_infinite(datapath, X_train, src_vocab_size, y_train, X_test, y_test, best_params, data_variants, device):
    ...
    max_seq_len = X_train.shape[1]
    # max_seq_len = best_params['max_seq_len']
    # create model
    model = EnsembledModel(src_vocab_size, max_seq_len, device, tuning_params=best_params).to(device)
